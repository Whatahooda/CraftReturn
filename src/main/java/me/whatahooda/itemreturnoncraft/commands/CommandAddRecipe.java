...
public class CommandAddRecipe implements CommandExecutor {
    ...
    private boolean passGuardCases(Player p, ItemStack mainHand, ItemStack offHand, String[] args) {
        if (mainHand.getAmount() == 0) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "You must hold an item in your main hand to represent the \"craft-item\"");
            return false;
        }
        if (offHand.getAmount() == 0) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "You must hold an item in your off hand to represent the \"return-item\"");
            return false;
        }
        if (mainHand.getType() == offHand.getType()) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "The \"craft-item\" and \"return-item\" must be different");
            return false;
        }
        ...
        if (args.length < 2) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "You must provide a name for this return recipe(no spaces)");
            return false;
        }

        if (args[0].equals("general") && ReturnableItemManager.getManager().getNamesGeneral().contains(args[1])) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "A general recipe already has that name");
            return false;
        }
        else if (args[0].equals("nbt") && ReturnableItemManager.getManager().getNamesNBT().contains(args[1])){
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "A nbt recipe already has that name");
            return false;
        }

        if (args[0].equals("general") && ReturnableItemManager.getManager().isGeneralReturnable(mainHand)) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "There already exists a general recipe with that craft-item named \"" + ReturnableItemManager.getManager().getGeneralName(mainHand) + "\"");
            return false;
        }
        else if (args[0].equals("nbt") && ReturnableItemManager.getManager().isNBTReturnable(mainHand)) {
            p.sendMessage(CraftReturnUtil.CRAFT_RETURN_TAG + ChatColor.RED + "There already exists a nbt recipe with that craft-item named \"" + ReturnableItemManager.getManager().getNBTName(mainHand) + "\"");
            return false;
        }
